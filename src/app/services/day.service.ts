...
export class DayService {
  ...
  updateDay(
    pDayID: number,
    pDayProperties: { date: Date; menuItemID: number; time: Time; comment: string }
  ): Observable<Day> {
  ...
  private fetchWeek(pWeekNumber: number): Observable<Day[]> {
    return this.http.get<Day[]>('http://localhost:8000/api/week/' + pWeekNumber).pipe(
      map((responseDataArray) => {
        const dayArray: Day[] = [];
